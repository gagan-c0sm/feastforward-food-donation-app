...
import { GoldAsterisk, GreenHerb, TinyLeaf } from "./decorators/Decorators";
...
export function RequestFood() {
  ...
  const fetchRequests = async () => {
    try {
      const data = await api.getFoodRequests();
      setRequests(data);
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const newRequest = await api.createFoodRequest(formData);
  ...
  return (
    <div className="h-full overflow-auto relative" style={{ backgroundColor: "var(--background)" }}>
      ...
      <div className="max-w-4xl mx-auto p-6 pb-20 relative z-10">
        ...
        {loading ? (
          <div className="text-center py-10 text-sm font-medium" style={{ color: "var(--muted-foreground)" }}>Loading requests...</div>
        ) : (
